...
## 6. The improvement flow (`improvement_flow.py`, `nodes/`)

Front-half that produces a draft and feeds the QA back-half:

1. `parse_request` (`nodes/parse_request.py`) → `ImprovementSpec` (operation, target slug, confidence) — **live (deterministic parse)**
2. `LessonIndex.hydrate` (`store/index.py`) → retrieval context over `data/lessons/` — **live**
3. `triage_request` (`nodes/triage.py`) → re-routes low-confidence/off-scope requests — **live**
4. `feasibility_check` (`nodes/feasibility.py`) → `feasible | infeasible | off_scope` — **live**
5. Author the draft by operation:
   - `add_exercises` (`nodes/add_exercises.py`) — **live author** via `construct_payload` + `call_with_validation` (the public `build_validated_exercise` helper)
   - `add_explanation` (`nodes/add_explanation.py`) — **live author**
   - `improve` → **live** whole-lesson revise author (`nodes/revise_lesson.py`, Task D)
...
## 7. The curriculum flow (`curriculum/flow.py`) — **partial**

`run_curriculum_flow` proposes a slug and drafts/derives `concept_requirements` for it;
`commit_curriculum_draft` copies an approved draft's requirements into
...
| Link | Status | Where |
|------|--------|-------|
| curriculum text → requirements | ✅ live (drafts requirements) | `curriculum/flow.py` |
...
| 8 | `improve` whole-lesson revise | **live** | `nodes/revise_lesson.py` (structured `Lesson` output, spec-driven) | — |
...
`scripts/migrate_import.py` is now a thin shim over the package. After import, Journey 3 takes over:
`improve --slug <slug>` / `graph run <slug>` already work on a registered lesson.
