...
# ---------------------------------------------------------------------------
# Ban-list loads + validates
# ---------------------------------------------------------------------------


class TestBansLoads:
    def test_load_terminology_bans_given_on_disk_markdown_expect_validates(self):
        # setup + exercise
        bans = load_terminology_bans()
        # assert: seeded from docs/terminology-style-guide.md
        assert "tense-carrying verb" in bans.banned_phrases
        assert "subjunction" in bans.banned_phrases
        assert "In this lesson, you will" in bans.prose_tells

    def test_load_terminology_bans_given_missing_file_expect_empty(self, tmp_path: Any):
        # setup
        missing = tmp_path / "nope.md"
        # exercise
        bans = load_terminology_bans(missing)
        # assert: no file -> empty ban list (not an error)
        assert bans.banned_phrases == []
        assert bans.prose_tells == []

    def test_write_terminology_bans_given_markdown_copy_expect_updates_fenced_block(self, tmp_path: Any):
        # setup
        guide = tmp_path / "terminology-style-guide.md"
        guide.write_text(
            "before\n```bans\nban: old phrase\nprose-tell-density: , meaning | , which means | 2\n```\nafter\n",
            encoding="utf-8",
        )
        bans = TerminologyBans(
            banned_phrases=["new phrase"],
            prose_tells=["In this lesson, you will"],
        )
        # exercise
        write_terminology_bans(guide, bans)
        loaded = load_terminology_bans(guide)
        # assert
        assert loaded.banned_phrases == ["new phrase"]
        assert loaded.prose_tells == ["In this lesson, you will"]
        assert "prose-tell-density" in guide.read_text(encoding="utf-8")


class TestActiveRulesLoads:
    def test_load_active_terminology_rules_given_on_disk_markdown_expect_pinned_terms(self):
        # setup + exercise
        rules = load_active_terminology_rules()
        # assert
        by_concept = {rule.concept: rule for rule in rules}
        assert by_concept["present tense"].pinned_term == "present tense"
        assert by_concept["finite verb"].pinned_term == "finite verb"

    def test_upsert_active_terminology_rule_given_markdown_copy_expect_updates_table(self, tmp_path: Any):
        # setup
        guide = tmp_path / "terminology-style-guide.md"
        guide.write_text(
            "| Concept | Pinned term | Also OK | Guidance / hard bans | Level note |\n"
            "|---|---|---|---|---|\n"
            "| present tense | **present tense** | — | — | all levels |\n"
            "\n## Rationale\nKeep prose.\n",
            encoding="utf-8",
        )
        rule = ActiveTerminologyRule(
            concept="present tense",
            pinned_term="present tense",
            also_ok="—",
            avoid_banned="`presens`",
            level_note="all levels",
        )
        # exercise
        upsert_active_terminology_rule(guide, rule)
        rules = load_active_terminology_rules(guide)
        # assert
        assert len(rules) == 1
        assert rules[0].avoid_banned == "`presens`"
        assert "Keep prose." in guide.read_text(encoding="utf-8")

    def test_upsert_active_terminology_rule_given_pipe_in_field_expect_value_error(self, tmp_path: Any):
        # setup
        guide = tmp_path / "terminology-style-guide.md"
        guide.write_text(
            "| Concept | Pinned term | Also OK | Guidance / hard bans | Level note |\n"
...
class TestBannedPhrase:
    def test_terminology_check_given_banned_surface_expect_warning(self):
        # setup
        bans = _test_bans()
        lesson = _lesson([
        ...
        assert banned[0].is_blocking is False
...
class TestProseTell:
    def test_prose_tell_density_threshold_constant_is_two(self):
        # setup + exercise + assert: the threshold is a linter constant
        assert K_PROSE_TELL_DENSITY_THRESHOLD == 2
    ...
    def test_terminology_check_given_single_appositive_expect_no_density_tell(self):
        ...
        density_tells = [
            r for r in results
            if r.check_id == K_TERM_CHECK_PROSE_TELL and "density" in r.message
        ]
    ...
    def test_terminology_check_given_literal_in_this_lesson_expect_warning(self):
        ...
        tells = [
            r for r in results
            if r.check_id == K_TERM_CHECK_PROSE_TELL and "In this lesson, you will" in r.message
        ]
        assert len(tells) >= 1
...
class TestNewProseTellPatterns:
    ...
    def test_form_appositive_density_flags_when_repeated(self):
        # setup: two ", the ... form" appositives hit the density threshold (>= 2)
        lesson = _lesson([
            _section("sec_1", [
                "Use the neuter form, the form for adverbs. "
                "Take the infinitive, the base form of a verb.",
            ]),
        ])
        # exercise
        results = terminology_check(lesson)
        # assert: density >= 2 -> at least one prose-tell finding
        tells = [r for r in results if r.check_id == K_TERM_CHECK_PROSE_TELL]
        assert len(tells) >= 1

    def test_form_appositive_single_not_flagged(self):
    ...
    def test_plus_notation_flagged_in_exercise_prompt(self):
        ...
        plus_tells = [
            r for r in results
            if r.check_id == K_TERM_CHECK_PROSE_TELL and "'+'-notation" in r.message
        ]
        assert len(plus_tells) == 1

    def test_plus_notation_not_flagged_in_section_rule(self):
