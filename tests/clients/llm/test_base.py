...
def test_extract_json_object_given_no_json_expect_type_error():
...
def test_extract_json_object_given_multiple_objects_expect_type_error():
    with pytest.raises(TypeError):
...
def test_matches_quota_given_matching_pattern_expect_true():
    class _Client(base_mod.BaseLlmClient):
        ...
        def call(self, prompt, *, model=None, agent=None, variant=None):
            return base_mod.LlmResponse(text="", client="t", model=model or "t")
