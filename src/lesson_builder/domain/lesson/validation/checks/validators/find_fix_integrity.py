...
from lesson_builder.domain.lesson.validation.checks.defect_rules import check_error_token_alignment
from lesson_builder.domain.lesson.validation.checks.defect_rules import collapse_ws
from lesson_builder.domain.lesson.validation.checks.defect_rules import extract_corrected_sentence
from lesson_builder.domain.lesson.validation.checks.defect_rules import normalize
...
def _find_fix_exercises(data: dict[str, Any]) -> list[dict[str, Any]]:
    return [
        ...
        if isinstance(el, dict) and el.get("element_kind") == "exercise" and el.get("operation") == "find_fix"
    ]
