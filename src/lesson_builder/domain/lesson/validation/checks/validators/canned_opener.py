...
implementation used by the deterministic gate) and mirrors the relevant
``explanation[0]`` walk.
"""
...
from lesson_builder.domain.lesson.validation.checks.defect_rules import is_canned_opener
...
def canned_opener_check(data: dict[str, Any]) -> list[CheckResult]:
    ...
    for el in data.get("elements", []):
        ...
        if not isinstance(explanation, list) or not explanation:
            continue
        first = explanation[0]
        if not (isinstance(first, dict) and isinstance(first.get("value"), str) and first["value"]):
            continue
