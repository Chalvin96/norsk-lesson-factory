...
The eligible-operation matrix in :mod:`lesson_builder.domain.lesson.models.operations` already
        ...
        if bloom not in targets:
            results.append(
                CheckResult(
                    ...
                    severity="warning",
                    unit_id=ex_id,
                    message=(f"exercise bloom_level '{bloom}' not in objective '{obj_id}' targets {targets}"),
                    fix_hint=(
        ...
        if unreached:
            results.append(
                CheckResult(
                    ...
                    fix_hint=(
                        f"Add at least one exercise at {unreached} for objective '{oid}', or drop the unreached target."
                    ),
                )
            )
...
def _objectives_by_id(data: dict[str, Any]) -> dict[str, dict[str, Any]]:
    return {obj["id"]: obj for obj in data.get("objectives", []) if isinstance(obj, dict) and obj.get("id")}
