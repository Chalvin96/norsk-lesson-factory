...
- answerable: every choose/recall_fill has a carrier stem, exactly one correct answer, plausible
  ...
  prompt/context is a serious answerability defect, even if the lesson prefers another form.
...
the lesson's cefr_level? The terminology style guide is authoritative: do not penalize a pinned \
or Also OK glossary term merely because it is technical. Standard textbook terms are acceptable \
even at A1/A2 when the lesson needs the concept and the term is introduced, scaffolded with a \
plain explanation, or already established in the lesson (e.g. "infinitive", "bare infinitive", \
"modal verb", "present tense"). Flag unexplained label-dumps, invented labels, or descriptions \
that are not actionable for the level (e.g. "front rounded vowel" with no English approximation \
at A1). At lower levels, prefer learner action / plain explanation near the first label; \
pronunciation guidance should give an English approximation + mouth-position note, not just \
