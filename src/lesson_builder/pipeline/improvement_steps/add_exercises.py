...
def _lesson_summary(lesson: dict[str, Any], *, max_examples: int = 3) -> str:
    """Curated lesson context: objectives + a few existing exercises.

    Replaces a byte-sliced ``json.dumps(...)[:N]`` that handed the model
    truncated, invalid JSON. We send whole, valid fragments instead.
    """
    objectives = lesson.get("objectives", [])
    existing = [e for e in lesson.get("elements", []) if e.get("element_kind") == "exercise"]
    summary = {
        "concept_slug": lesson.get("concept_slug", ""),
        "objectives": objectives,
        "existing_exercises_sample": [
            {"operation": e.get("operation"), "prompt": e.get("prompt"), "objective_id": e.get("objective_id")}
            for e in existing[:max_examples]
        ],
    }
...
def _build_author_prompt(spec: ImprovementSpec, lesson: dict[str, Any]) -> str:
    ...
    # which imports this module — a top-level import here would deadlock the
    # cycle when add_exercises is imported first.
    ...
    return (
        ...
        "`explanation_text`, and a `flat` object carrying the operation-specific fields. "
        "Python owns every id, index, and structural flag — you only provide the content.\n"
        f"Return ONLY a JSON object: {{\"exercises\": [{{\"operation\": \"choose\", "
