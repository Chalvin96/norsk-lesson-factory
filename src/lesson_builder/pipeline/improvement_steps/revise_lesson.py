...
def _build_prompt(spec: ImprovementSpec, lesson: dict[str, Any]) -> str:
    return (
        ...
        "Preserve the lesson key, objectives, and overall shape, and keep every required anchor "
        "form. Return the COMPLETE revised lesson.\n\n"
        "Apply only changes the request calls for. Where you do change teaching prose, make it "
        "concrete: teach the pattern with a short why and a worked example rather than listing "
        "rules; gloss any grammar term on first use; keep learner-facing register natural and "
        "level-appropriate. Where you touch an exercise that offers competing options "
        "(choose/recall_fill), its explanation must name each distractor and the meaning or form "
        "it would signal, not just bless the correct answer. Do not open explanations with an "
        "acknowledgment word like Correct/Good/Right/Yes.\n\n"
