"""Shared result type. Not a check itself — every check in this package returns
``list[CheckResult]`` (or a tuple wrapping one, for advisory checks)."""
...
class CheckResult(BaseModel):
    ...
    advisory   — True for uncalibrated LLM judges (log-only until threshold config promotes the judge)
