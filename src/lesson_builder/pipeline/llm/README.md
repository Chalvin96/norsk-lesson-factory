...
`openrouter.py` (`OpenrouterClient`) is wired into `default_clients()` but
not yet used by any `Agent` profile in `pipeline/agents.py` — it's staged
for the next cycle's work, not dead code. Don't remove it just because
nothing points at it yet; do remove it if a future cycle confirms the plan
